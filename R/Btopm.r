...
function(formula, tau=0.5, family= "rq", Ce=0, runs=15000, burn=1000) {
    ...
    if (all(x[,2]==1)) x=x[,-2] }
      ...
      # Saving output matrices 
        ...
        Cor.yx=c(abs(cor(y,Xc)))
        Xc=as.matrix(Xc)
        if (ncol(Xc==1)) ST= sd(c(Xc))/sd(c(y)) else{
    ...
    # Draw the latent v from inverse Gaussian distribution.
      ...
      # Draw  beta 
       ...
       if (family== "bin") (C=0.5) else(C=0.2)
      Index =c(abs(beta*Cor.yx)/sqrt(2*Lamb) + beta^2* ST + abs(beta)/sqrt(2*Lamb))        
       Zero=which(Index<= C)
